...
$result = $wpdb->get_results("SELECT * FROM ocpn_nga_charts A JOIN ocpn_nga_charts_links B ON A.number = B.number LEFT JOIN ocpn_nga_charts_with_params C ON (A.number = C.number) WHERE A.number >= 0 AND A.number <= 9999 ORDER BY A.number ASC", ARRAY_A);
